...
module.exports = function(Polyglot, polyInterface) {
  ...
  class JishiInterface {
    ...
    async playerPlaylist(group, playlist) {
      let data = this.axiosGet(group + '/playlist/' + playlist);
      return data;
    }

    async playerSay(group, say) {
      let data = this.axiosGet(group + '/say/' + say);
      return data;
    }

    async playerSayAll(say) {
      let data = await this.axiosGet('sayall/' + say);
      return data;
    }

    async playerClip(group, clip) {
      let data = this.axiosGet(group + '/clip/' + clip);
      return data;
    }

    async playerClipAll(clip) {
      logger.info('Clip All Clip from API: %s', clip);
      let data = await this.axiosGet('clipall/' + clip);
      return data;
    }

    async playerJoin(player, group) {
      let data = this.axiosGet(player + '/join/' + group);
      return data;
    }

    async playerLeave(player) {
      let data = this.axiosGet(player + '/leave');
      return data;
    }
